    ...
    gen move {
        while let Some((cmd, rest)) = line.split_first() {
            ...
            match (
                ...
                (true, read, _, true) => {
                    let Some(([len, data], rest)) = line.split_first_chunk() else {
                        break;
                    };
                    line = rest;
                    yield y(
                ...
                (false, true, false, true) => {
                    let Some((len, rest)) = line.split_first() else {
                        break;
                    };
                    line = rest;
                    yield y(
                ...
                (false, read, true, false) => {
                    let Some(([len_low, len_high], rest)) = line.split_first_chunk() else {
                        break;
                    };
                ...
                _ => panic!("unknown command: {cmd:02X}"),
            }
        }
    }
}
