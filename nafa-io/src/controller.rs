    ...
    loop {
        ...
        match u32::from_le_bytes(id) {
            ...
            idcode if idcode & 0xfff == (0x093 << 1) && ret.is_empty() => {
                buf.clear();
                let idcode = zynq_us_init_arm_dap(backend, buf)?;
...
fn zynq_us_init_arm_dap<B: Backend>(backend: &mut B, buf: &mut Vec<u8>) -> Result<u32> {
    ...
    backend.tms(buf, Path::RESET)?;
    backend.bits(buf, reset_to_sir, jtag_ctrl, Bits(16), sir_to_idle)?;
    backend.bytes(buf, idle_to_sdr, ones, sdr_to_reset)?;
    backend.bytes(buf, reset_to_sdr, ones, sdr_to_reset)?;
    backend.bytes(buf, reset_to_sdr, rx_4, sdr_to_idle)?;
    backend.flush(buf)?;
